...
if($_POST['btnhid']=="Save"){
		 ...
		 $sqli="INSERT INTO ".DB_DATABASE.".ebs_customer_master 
				SET  company_name='".$_POST['cname']."', customer_description='".$_POST['desc']."', 
		...
		$upud=$_POST['idcus'];
		 $sqli="UPDATE lgt_ebs_generate.ebs_customer_master SET company_name='".$_POST['cname']."', 
				customer_description='".$_POST['desc']."', emp_update='$user_login', date_update='".date('Y-m-d H:i:s')."' 
...
<table width="98%"  border="1" class="table01" align="center">
  ...
  <tr>
    ...
    <td width="780"><div class="tmagin_right">
      <input type="text" name="cname" id="cname" size="45" value="<?=$rs['company_name']?>"/>
    </div></td>
  </tr>
  <tr>
    <td width="337" height="25"><div class="tmagin_left">Description  :</div></td>
    <td width="780"><div class="tmagin_right">
      <textarea name="desc" id="desc" cols="40" rows="3"><?=$rs['customer_description']?></textarea>
      </div></td>
  </tr>
   <tr>
    <td width="337" height="25"><div class="tmagin_left">Ststus Generate :</div></td>  
