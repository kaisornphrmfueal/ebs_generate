	...
	if(!empty($_POST['hbtn']) AND $_POST['hbtn']=="Confirm"){
		...
		$pgroup=$_POST['hgroup'];
				  ...
				  /*	$sqlc="SELECT a.group_id,a.order_no,a.delivery_date,a.cpo_date,a.customer_part_no,
							...
							*/
		$sqlc="SELECT a.group_id,a.order_no,a.delivery_date,a.cpo_date,a.customer_part_no,a.customer_name,
							a.delivery_qty,a.id_destination,b.customer_code,b.ship_name,c.pass_thru_status,b.currency,b.ship_method,
							CASE c.pass_thru_status WHEN '0' THEN 'FTTL_FG_DOMESTIC'  WHEN '1' THEN 'FTTL_FG_PASS_THRU' ELSE 'Empty' END AS sptrue,a.supp_code,a.plant_name,
							DATE_FORMAT(a.delivery_date, '%d%m%Y') AS delivery_d,
							DATE_FORMAT(a.cpo_date, '%d%m%Y') AS cpod ,
    						CASE  WHEN b.customer_code IS NULL THEN '1'    ELSE '0' END AS chk_error,
							b.gms_delivery_place_code,b.price_list
							FROM ".DB_DATABASE.".ebs_order_original a
							LEFT JOIN ebs_formula_cal d ON (a.supp_code = d.supplier_code AND a.customer_name = d.car_plant 
                                                           AND a.plant_name = d.dock_code)
							LEFT JOIN ".DB_DATABASE.".ebs_customer_destination b ON d.id_destination=b.id_destination
							LEFT JOIN ".DB_DATABASE.".ebs_part_master c ON a.customer_part_no  = c.customer_part
							WHERE a.group_id='$pgroup' 
							GROUP BY a.id_original";
					...
					if(mysqli_num_rows($qrc)<>0){
							...
							while($rsi=mysqli_fetch_array($qrc)){
								$sqli="INSERT INTO ".DB_DATABASE.".ebs_report SET group_id='".$rsi['group_id']."', 
									...
									gms_delivery_place_code='".$rsi['gms_delivery_place_code']."', price_list='".$rsi['price_list']."', 
...
<div align="center">
    <?php
			  $gidst=$_GET['cidg'];
			   $gcust=$_GET['cusid'];
					//// comment  pass_thru_status update new destination why?
					/*if($gcust=="1"){
...
<form id="form1" name="form1" method="post" action=""onsubmit='return validate(this)'>
    ...
    <tr>
     <th height="30" colspan="15">Order imported confirmation 
    ...
    <?php 	while($rs=mysqli_fetch_array($qr)){ 
   	   ...
   	   <tr  <?php $v =0; $v = $v + 1; echo  icolor($v); ?>  onMouseOver="className=&quot;over&quot;"  onMouseOut="className=&quot;&quot;" height="25px" align="center" >
   	     ...
   	     <td <?php if($rschk_error=="1"){echo "bgcolor='#FF0000'";}?> > <?php echo sprintf("%05d", $rs['customer_code']);?> </td>
   	     <td><?php echo $rs['ship_name'];?></td>
   	     <td><?php echo $rs['supp_code'];?></td>
   	     <td><?php echo $rs['customer_name'];?></td>
   	     <td><?php echo $rs['plant_name'];?></td>
    ...
    ?>
         ...
         <?php 	if(selectCountNull($gidst)<>0){?>
			...
			<?php	}elseif($total_error<>0) {?>
		
		 <input type="submit" name="button4" id="button4" value="Add New Formula"
             onClick="javascript:openWins('windows.php?win=addcal', '_blank', 650, 350, 1, 1, 0, 0, 0);return false;"  />
         ...
         onclick="window.location.href='index.php?cid=<?=base64_encode("1_cal")//$gcust ?>';"  /></td>
   	     </tr>
