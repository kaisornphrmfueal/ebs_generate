...
//--------------------------START 21 IMCT SERVICE ---------------------------
		...
		log_hist($user_login,'Add','ebs_group_order',$groupig);
			...
			$npwd=addslashes($objArr[7]);
			    $csql="INSERT INTO ".DB_DATABASE.".ebs_order_original SET  group_id='$groupig',invoice_no='".$objArr[0]."',id_destination='$dest',
					...
					customer_part_no='".$objArr[6]."', pwd='".$npwd."', fttl_part_no='".$objArr[8]."', part_name='".$npname."',
	...
	if($_POST['btnNormalSubmit']=="Submit"){
		...
		log_hist($user_login,'Add','ebs_group_order',$groupig);
			while (($objArr = fgetcsv($objCSV,2000, "\t")) !== FALSE) {  //,
			if(!empty($objArr[0])){
			 @$daten3=convDate2($objArr[2]);
			// echo $daten3;
			$npart=substr($objArr[1], 0, 6)."-".substr($objArr[1], -4);  ;
			$daten13= date("Y-m-d", strtotime($daten3." -2 day" ) );
       ...
       <br/>
            ...
            <img src="exp/<? echo "imct"?>.png"   />
             ...
             <div id="fragment-3">
     			  <br/>
                        Download Format to import  Text file ::  
						<?php echo "<a target='_blank' href=exp/IMKDCIMCT.txt >Exp. Isuzu IMKDC  File (IMKDCIMCT.txt)</a>";?>  
              <br/> <br/>
			...
			<br/>
            <img src="exp/<? echo "imkdcimct"?>.png"/>
             ...
             <div id="fragment-4">
                  ...
                  <br/>
                            Download Format to import  Text file ::  
						<?php
						 echo "<a target='_blank' href=exp/IMCTGW.txt >Exp. IMCT Gateway File (IMCTGW.txt)</a>";
