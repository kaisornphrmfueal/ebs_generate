	...
	if($_POST['hbtn']=="Confirm"){
		...
		$pgroup=$_POST['hgroup'];
				  	...
				  	$sqlc="SELECT a.group_id,a.order_no,a.delivery_date,a.cpo_date,a.customer_part_no,
							a.delivery_qty,a.id_customer_master,b.customer_code,b.ship_name,c.pass_thru_status,b.currency,b.ship_method,
							...
							LEFT JOIN ".DB_DATABASE.".ebs_customer_destination b ON a.id_customer_master=b.id_master
					...
					if(mysql_num_rows($qrc)<>0){
							log_hist($user_login,'Confirm','ebs_report',$pgroup);
							mysql_query("UPDATE lgt_ebs_generate.ebs_group_order SET status_group='Confirm', 
										emp_update='$user_login', date_update='".date('Y-m-d H:i:s')."' WHERE group_id='$pgroup'");
							while($rsi=mysql_fetch_array($qrc)){
...
<div align="center">
    <?
			$gidst=$_GET['cidg'];
				 ...
				 echo 	$sql="SELECT a.group_id,a.order_no,a.delivery_date,a.cpo_date,a.customer_part_no,
...
<form id="form1" name="form1" method="post" action="">
    ...
    <tr>
     <th height="30" colspan="10">Order imported confirmation</th>
     </tr>
    <tr>
      ...
      <th width="9%">CPO DATE</th>
      <th width="9%">Customer Requested Date</th>
      <th width="7%">Order Qty.</th>
      <th width="14%">Customer Part No.</th>
    ...
    <?php 	while($rs=mysql_fetch_array($qr)){  ?>
   	   <tr  <?php   echo icolor($v); $v = $v + 1; ?> onMouseOver="className=&quot;over&quot;"  onMouseOut="className=&quot;&quot;" height="25px" align="center" >
   	     ...
   	     <td><div  class="tmagin_right"> <?php echo $rs['customer_code'];?></div></td>
   	     <td><span class="tmagin_right"><?php echo $rs['ship_name'];?></span></td>
   	     <td><span class="tmagin_right"><?php echo $rs['order_no'];?></span></td> 
   	     <td><?php echo sprintf("%06d", $rs['cpod']);?></td>
   	     <td><?php echo sprintf("%06d", $rs['delivery_d']);?></td>
   	     <td><?php echo $rs['delivery_qty'];?></td>
   	     <td><div  class="tmagin_right"><? echo $rs['customer_part_no'];?></div></td>
   	     <td><?php echo $rs['currency'];?></td>
   	      <td <? if($rs['sptrue']=="Empty"){echo "bgcolor='#FF0000'";}?>><span class="tmagin_right"><?php echo $rs['sptrue'];?></span></td>
   	     </tr>
          <?php						
		$i++;	
			}
    ...
    ?>
         ...
         <? 	if(selectCountNull($gidst)<>0){?>
            <input type="submit" name="button3" id="button3" value="Add Customer Part No." onClick="javascript:openWins('windows.php?win=addm', '_blank', 650, 350, 1, 1, 0, 0, 0);return false;"  />
			<?	}else{	 ?>
