...
//idcus
		    ...
		    $sqli="INSERT INTO ".DB_DATABASE.".ebs_part_master SET 
				...
				plant='".$_POST['plant']."', pass_thru_status=".$_POST['spass'].", 
		...
		$idp=$_POST['hidm'];
		  $sqli="UPDATE  ".DB_DATABASE.".ebs_part_master SET fttl_part='".$_POST['fttlpart']."', 
				customer_part='".$_POST['custpart']."', description='".$_POST['desc']."',
				plant='".$_POST['plant']."', pass_thru_status='".$_POST['spass']."', 
...
if(!empty($_GET['ide'])){
	$idg=$_GET['ide'];
 $sql="SELECT id_part,fttl_part,customer_part,description,pass_thru_status,plant,plant
...
<table width="98%"  border="1" class="table01" align="center">
  <tr>
    ...
    <td width="780"><div class="tmagin_right">
      <input type="text" name="custpart" id="custpart" size="45" value="<?php if(!empty($rs['customer_part'])){echo $rs['customer_part'];}?>"/>
   <span class="Arial_14_red">*</span> </div></td>
  </tr>
  <tr>
    <td width="337" height="25"><div class="tmagin_left">FTTL Model No. :</div></td>  
    <td width="780"><div class="tmagin_right">
      <input type="text" name="fttlpart" id="fttlpart" size="45" value="<?php if(!empty($rs['fttl_part'])) {echo $rs['fttl_part'];}?>"/>
      <span class="Arial_14_red">*</span></div></td>
  </tr>
  ...
  <tr> 
    ...
    <td width="780"><div class="tmagin_right">
      <textarea name="desc" id="desc" cols="40" rows="3"><?php if(!empty($rs['description'])){echo $rs['description'];}?></textarea>
      </div></td>
  </tr>
   </tr>
    <tr>
    ...
    <td width="780"><div class="tmagin_right">    
    <input type="text" name="plant" id="plant" size="45" value="<?php if(!empty($rs['plant'])){echo $rs['plant'];}?>"/> </div></td>
  </tr>
    <tr>
    <td width="337" height="25"><div class="tmagin_left">Status part. :</div></td>  
    <td width="780"><div class="tmagin_right">
      <select name="spass" id="spass">
        <option value="0"<?php if(!empty($rs['pass_thru_status']) AND $rs['pass_thru_status']==0){ echo "selected='selected'";}?> >FTTL_FG_DOMESTIC</option>
        <option value="1"<?php if(!empty($rs['pass_thru_status']) AND $rs['pass_thru_status']==1){ echo "selected='selected'";}?> >FTTL_FG_PASS_THRU</option>
      </select>
    </div></td>
  </tr>
